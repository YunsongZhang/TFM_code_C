...
#define PSO_INERTIA 0.7298 // default value of w (see clerc02) // === NEIGHBORHOOD SCHEMES ===
...
typedef struct {
    ...
    int numstalls;  // number of stalls
    ...
    int flag;  //flag used in the MATLAB PSO algorithm

    int maxStalls;  // The pso will stop after being stuck for maxStalls times
    int numAttempt;   // number of attempt going on
    int maxAttempts; // Once a pso attemp fails, the code will redo until maxAttempts have made
